...
3. **Dedupe**, in two passes, because the corpus has two distinct defects. First, 12 folders carry a **byte-identical transcript** under a different video id and title (an upstream copy error, usually a `guest-20`/`guest-30` folder); we cannot tell which metadata is right, so the unsuffixed slug wins. Then, 31 `video_id`s appear under more than one folder; the folder whose slug matches the title-derived guest wins, tie-broken on transcript length, and 23 of those folders are dropped (the rest were already removed by the first pass). `guest` is taken from the title suffix when it looks like a person's name, else frontmatter with "N.0" version suffixes stripped. Result: **303 folders − 35 duplicates = 268 episodes**.
