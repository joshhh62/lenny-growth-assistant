...
Both runtimes consume the **same tool registry** (`agent/tools.py`) — schema and handler defined once; the SDK runtime wraps handlers with `@tool` + `create_sdk_mcp_server`. `AGENT_RUNTIME=agent_sdk|messages_loop|auto` overrides the default for evaluation.
