...
| A2 | **The corpus has data-quality issues we must handle, not hide.** Discovery found 35 folders that duplicate another episode under the wrong slug/guest (e.g. `chip-conley/` contains Maggie Crowley's episode; `julian-shapiro/` contains Julie Zhuo's), and frontmatter `guest` values like "Elena Verna 4.0". | Wrong attribution destroys the trust the product exists to create. | We dedupe in two passes — identical transcript bodies, then `video_id` — derive the guest from the title suffix when it is a person's name, and strip version suffixes. 268 unique episodes remain. Documented in `architecture.md → Ingestion`. |
...
| AC12 | 3 timestamp formats parse and the dedupe rule keeps the right folder — unit-tested on fixtures (`test_ingest.py`); all 303 source folders parsing and collapsing to 268 episodes / 27,454 chunks is evidenced by the ingest run log, not by a test | `test_parse_*`, `test_dedupe_by_video_id_keeps_matching_slug`; ingest run log in [agent-transcripts/03-verification-runs.md](agent-transcripts/03-verification-runs.md) |
