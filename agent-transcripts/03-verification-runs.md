...
Desktop 1440×900 and mobile 400×820, driven end-to-end (new chat → grounded answer → essay → HTML artifact → drawer/sheet) against the **fake LLM**, so the run is deterministic and needs no model. Console errors: none after fixes.

All three screenshots in the README and design.md — [`desktop`](../docs/screenshot-desktop.png), [`essay`](../docs/screenshot-essay.png), [`mobile`](../docs/screenshot-mobile.png) — are from a **real session** on the reference laptop: `claude-sonnet-5` via the Agent SDK (and `qwen2.5:7b` locally for the narrow-width one), the full 268-episode corpus at 100 % embedded, genuine guests and timestamps. The earlier versions came from the scripted run and showed `fake-model` with fixture text, which misrepresented the product on the first screen of the README.
